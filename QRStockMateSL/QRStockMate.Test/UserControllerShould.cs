...
namespace QRStockMate.Test
{
	public class UserControllerShould
	{
		...
		[Test]
		public async Task Registro_Creates_User_And_Company_With_Generated_Code()
		{
			// Arrange
			var model = new RegistrationModel
			{
				User = new UserModel { Email = "newuser@example.com", Password = "password" },
				Company = new CompanyModel { /* Crear una instancia de CompanyModel */ }
			};
			_userServiceMock.Setup(x => x.getUserByEmailPassword(model.User.Email, It.IsAny<string>())).ReturnsAsync((User)null);
			_userServiceMock.Setup(x => x.getDirectorByCode(model.User.Code)).ReturnsAsync(new User());
			...
			var createdUser = (UserModel)createdAtActionResult.Value;
			createdUser.Code.Should().NotBeNullOrEmpty();
		}
		...
		public async Task GetCompanyByUser_Returns_OkObjectResult_With_CompanyModel()
		{
			...
			var company = new Company { /* Crear una instancia de Company para simular el retorno del servicio */ };
			_userServiceMock.Setup(x => x.getCompany(user.Code)).ReturnsAsync(company);
			...
			okObjectResult.Value.Should().BeEquivalentTo(company, options => options.ExcludingMissingMembers());
		}
